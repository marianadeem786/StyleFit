...
export default function SuggestMatchingScreen({ navigation }) {
  ...
  const handleImagePick = async () => {
    ...
    if (!result.canceled) {
      ...
      fetchSuggestedMatches(result.assets[0].uri);
    }
  };

  const fetchSuggestedMatches = async (imageUri) => {
    ...
    try {
      const response = await fetch(`${config.BACKEND_URL}/api/suggest_match_view/`, {
        method: 'POST',
        body: formData,
      });
      ...
      if (response.ok) {
        ...
        if (data.results && data.results.length > 0) {
          ...
          Alert.alert('No results found', 'No matching products were found.');
        }
      } else {
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <View style={styles.titleContainer}>
        <Text style={styles.title}>SUGGEST MATCHING</Text>
      </View>
      ...
      {results && results.length > 0 && (
        <View style={styles.resultsContainer}>
          <Text style={styles.resultsLabel}>Suggested Matches:</Text>
          {results.map((item, index) => (
            <View key={index} style={styles.resultCard}>
              ...
              <TouchableOpacity onPress={() => openURL(item.url)}>
                <Text style={styles.resultPrice}>{item.url}</Text> {/* URL is now clickable */}
              </TouchableOpacity>
            </View>
          ))}
        </View>
